...
class KeyValTable(QTableWidget):
    def __init__(self, parent=None):
        ...
        self.key_header.setFlags(self.key_header.flags() & ~Qt.ItemIsEditable & ~Qt.ItemIsSelectable)
        self.val_header.setFlags(self.val_header.flags() & ~Qt.ItemIsEditable & ~Qt.ItemIsSelectable)
        self.left_corner.setFlags(self.left_corner.flags() & ~Qt.ItemIsEditable & ~Qt.ItemIsSelectable)
        self.right_corner.setFlags(self.right_corner.flags() & ~Qt.ItemIsEditable & ~Qt.ItemIsSelectable)
        self.add_row_if_last(0)

        
        self.horizontalHeader().hide()
        self.verticalHeader().hide()
        self.setColumnWidth(CHECKBOX_COL, CHECKBOX_WIDTH)  # Fixed width of 100 pixels
        self.setColumnWidth(XBUTTON_COL, XBUTTON_WIDTH)
        # Set resize mode for other columns
        self.horizontalHeader().setSectionResizeMode(KEY_COL, QHeaderView.Stretch)  # Expandable column 1
        self.horizontalHeader().setSectionResizeMode(VALUE_COL, QHeaderView.Stretch)  # Expandable column 2
    ...
    def is_empty_row(self, row):
        return not self.item(row, KEY_COL).text() and not self.item(row, VALUE_COL).text()
    ...
    def add_row_if_last(self, row=None):
        if row == self.last_row and not self.is_empty_row(row):
            ...
            checkbox.setFlags(checkbox.flags() & ~Qt.ItemIsEditable & ~Qt.ItemIsSelectable)
            key = QTableWidgetItem()
            # key.setFlags(key.flags() | Qt.ItemIsEditable & ~Qt.ItemIsSelectable)
            ...
            self.last_row += 1
            # WARNING IT USES NEW LAST_ROW (INCREMENTED
    ...
    def add_x_button(self, row, col=XBUTTON_COL):
        ...
        item_in_row = self.item(row, 2) # Could be any other non widget item in button's row
